...
create_categorical_details <- function(data, mapping) {
  ...
  result <- categorical_cols %>%
    summarise(across(everything(), get_ratio_separate)) %>%
    pivot_longer(everything(),
      names_to = "raw_field_name",
      values_to = "values"
    ) %>%
    group_by(raw_field_name) %>%
    mutate(col_num = rep(1:10, length.out = n())) %>% # Changed from 1:20 to 1:10
    pivot_wider(
    ...
    select(preferred_field_name, starts_with("value_"))
